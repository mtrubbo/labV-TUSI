...
<body style="background-color: #10ba99;">
	...
	<section class="sectionTable" style="margin-top:8rem">
	...
	<!-- Action Modal -->
	<button type="button" class="btnNewStock btnNewArt"
		data-bs-toggle="modal" data-bs-target="#newStock">Registrar
		Ingreso de Stock</button>
	...
	</section> <!-- END DATATABLE --> <!-- MODALS --> <!-- Modal NUEVO STOCK -->
	<div class="modal fade" id="newStock" tabindex="-1"
		aria-labelledby="newStocklabel" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="newStocklabel">Registrar Ingreso
						de Stock</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal"
						aria-label="Close"></button>
				</div>
				<div class="modal-body">
					<form action="${pageContext.request.contextPath}/stocks/adding"
						method="GET">

						<div class="row">
							<div class="col-md-6">
								<label class="form-label">Articulo</label> <select id="articulo"
									name="articulo" class="form-control">
							...
							<div class="col-md-6">
								<label class="form-label">Fecha de Ingreso</label> <input
									id="fechaIngreso" type="date" name="fechaIngreso"
						...
						<div class="row">			
							<div class="col-md-6">
								<label class="form-label">Cantidad</label> <input id="cantidad"
									type="number" min="0" name="cantidad" class="form-control"
									required>
							</div>
							
							<div class="col-md-6">
								<label class="form-label">Precio de Compra</label> <input
									id="precioCompra" type="number" min="0" name="precioCompra"
									class="form-control" required>
							</div>
						</div>
						...
						<div class="mt-5">
							...
							<button type="submit" class="btn btnNewArt">Ingresar
								Stock</button>
						</div>
					</form>
				</div>
	...
	<script>
				...
				.ready(
						function() {
							...
							$('#tableStocks').DataTable({
								order : [ [ 0, 'desc' ] ]
							// Ordena por la primera columna (ID) de forma descendente
							});
												...
												$
														.ajax({
															...
															success : function(
																...
																if (res.status == 'ok') {
																	mostrarNotificacionYRecargar(res.message
																			+ ". Refrescando sitio...");
																}
																if (res.status == 'error') {
		...
		function mostrarNotificacionYRecargar(mensaje) {
			...
			toastr.options = {
				...
				timeOut : 2000
			// Duración de la notificación en milisegundos (3 segundos en este caso)
			};
